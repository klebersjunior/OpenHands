...
- Conversation **Desktop** tab (XFCE + KasmVNC): on-demand GUI inside the Docker `agent-canvas` image. KasmVNC binds loopback only (`ports.desktopVnc`, default 6901). Access is exclusively via `scripts/desktop-proxy.mjs` at `/api/desktop` (HTTP + authenticated WebSocket upgrade on ingress/static-server). Lifecycle: `POST /api/desktop/start` → cookie → iframe. Outside Docker the tab shows an unavailable empty state. Spec: `docs/spec-browser-desktop.md`.
...
- Electron desktop app (`npm run desktop` for dev / `npm run build:desktop` for the binary) starts the same stack as `dev-with-automation.mjs` but inside an Electron BrowserWindow. Three gotchas live here:
  ...
  3. **Busy preferred ports**: CLI launchers still fail fast via `assertPortsFree` when 8000 / 18000 / 18001 / 3001 (or env overrides) are taken. The desktop app instead runs `ensureDesktopPortsAvailable()` before `main()`: `scripts/desktop-port-remap.mjs` + `findFreePorts` propose free alternatives, a native dialog asks the user to **Use alternate ports** or **Quit**, and on accept the chosen values are written into `PORT` / `OH_CANVAS_SAFE_BACKEND_PORT` / `OH_CANVAS_SAFE_AUTOMATION_PORT` / `OH_CANVAS_SAFE_VITE_PORT` so `buildConfig` proceeds. The splash wait and main `BrowserWindow` must use the resolved `config.ingressPort` (not a hardcoded `localhost:8000`). Remap planning helpers are unit-tested in `__tests__/scripts/desktop-port-remap.test.ts`.

- **Pentest Electron IPC (PROJETOSIN-193):** host Docker Compose + ADB hooks live under `electron/pentest-ipc/` and `electron/pentest-preload.cjs` (`window.pentestNative`). Default **off** — set `PENTEST_ELECTRON_HOOKS_ENABLED=true` only for local desktop (`PENTEST_ELECTRON_HOOKS_ENABLED=true npm run desktop`). Disabled in `--public` / server-only mode (handlers return `hooks_disabled` / 403). Allowlisted argv only; compose `projectDir` must sit under `config/defaults.json` → `pentest.electronComposeRoots` (plus `OH_CANVAS_SAFE_STATE_DIR`). Do **not** merge this preload with loading-splash `boot-log:*` / `desktopBoot`. Unit tests: `__tests__/electron/`. Spec: `docs/specs/fase-3/193-electron-docker-adb-ipc.md`.
